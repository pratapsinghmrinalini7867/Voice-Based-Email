    ...
    return (
        <>
            ...
            <Box sx={{ flexGrow: 1, overflow: 'hidden', px: 2 }}>
                    ...
                    <Item sx={{my: 2, mx: 'auto', py: 2, px: 5, }} style={{maxWidth: "800px"}} key={item._id}>
                        ...
                        <Typography noWrap style={{maxWidth: "100px"}}>{item.subject}</Typography>
                        <Typography noWrap onClick={() => openMail(item._id)} style={{ textDecoration: "none"}}>{item.description}</Typography>
                        <p style={{textAlign: "right"}}>{item.date_recieved.$date}</p>
                        </Stack>
                    </Item>
                    </a>
                    ))}                
